...
/*
  ...
  and optional initial value as a parameter and returns a single value. It is a good practice to define an initial value for the accumulator value. 
  If we do not specify this parameter, by default accumulator will get array first value. If our array is an empty array, then Javascript will throw an error.
*/
...
const countriesLand = countries.filter(country => country.toLowerCase().includes('land'))
...
const countriesSixAndMore = countries.filter(country => country.length >= 6)
...
const productPricesWithValue = products.filter(product => typeof product.price === "number")
...
let sum = numbers.reduce((sum, currenNum) => sum += currenNum)
console.log(sum)

// 18 Use reduce to concatenate all the countries and to produce this sentence: Estonia, Finland, Sweden, Denmark, Norway, and IceLand are north European countries

let countriesText = countries.reduce((text, currenCountry) =>{
  if(currenCountry != "IceLand"){
    text +=", " + currenCountry
  }
  else{
    text +=`, and ${currenCountry} are north European countries`
  }
  return  text
})
...
console.log(products.filter(product => typeof product.price === "number").map(product => product.price).reduce((sum, current) => sum + current))
